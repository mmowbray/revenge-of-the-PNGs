...
public class Game extends ApplicationAdapter
{
	...
	enum GameState { Normal, Intermission, GameOver , Paused};
	GameState currentGameState, previousGameState;

	int wave, toSpawn, toKill, score;
	...
	public void create ()
	{
		...
		intermissionLength = 5800; //5.8 seconds
	...
	public void render ()
	{
		/*needs to be re-thought out, as gameTime is being updated even in paused gameState*/
		gameTime = TimeUtils.timeSinceMillis(gameStartTime);

		if (gameTime - lastPress > pressDelay && Gdx.input.isKeyPressed(Keys.Z)){
		...
		switch (currentGameState)
		{
			case Normal:
			{
				...
				for (int i = 0; i < enemies.size(); i++)
				{
					...
					if (enemies.get(i).alive) //wasn't previously killed by colliding with player
					{
						for (int j = 0; j < projectiles.size(); j++) //check against all projectiles
						{
							...
							{
								if(enemies.get(i).lostHealth(1))
								{
									score += enemies.get(i).speed;
								}
				...
				if (toSpawn > 0 && gameTime - lastMonster > monsterDelay) { //add enemy
				...
				if (!player1.reloading && gameTime - lastShot > fireDelay)
				{
			...
			case Intermission:
			{
				if (gameTime - intermissionStart > intermissionLength) { //intermission is over
		...
		switch (currentGameState)
		{
			...
			case Paused:
			{
				font.draw(spriteBatch, "Game Paused!!!", Gdx.graphics.getWidth() / 2 - 120, Gdx.graphics.getHeight() - 5);
				break;
			}
		...
		font.draw(spriteBatch, "Score:" + score, Gdx.graphics.getWidth() - 180, Gdx.graphics.getHeight() - 5);
	...
	{
		currentGameState = GameState.Intermission;
		player1 = new Player(new Vector2(((Gdx.graphics.getWidth() - (Player.texture.getWidth() / 21)) / 2), ((Gdx.graphics.getHeight() - Player.texture.getHeight()) / 2) + 42), 7);
